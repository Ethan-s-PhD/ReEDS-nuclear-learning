"""
Ported to the clean nuclear-learning fork (upstream ReEDS layout) from
ReEDS-hybrid-plant. Storage-hybrid pooling/TES-island switches now default to
OFF (the clean fork carries no storage-hybrid machinery; the guarded
storage_hybrid_* input reads degrade gracefully when the files are absent).

Endogenous nuclear learning — between-years engine.

Runs once per solve year (scheduled by runreeds.setup_sequential, immediately
after 1_tc_phaseout.py and before the GAMS solve), mirroring the 1_tc_phaseout.py
pattern: read a small per-year gdx written by the previous solve year, compute
updated inputs, and write a gdx that 3_solve_oneyear.gms re-reads before the
2_financials.gms include.
...
cost recomposition in 3_solve_oneyear.gms — to storage-hybrid wrappers whose
...
def main(cur_year, case):
    ...
    # No OCC learning before the anchor year; 3_solve_oneyear.gms also skips the load.
    if cur_year < anchor:
        return
    ...
    pool_hybrid = int(sw.get('GSw_NuclearLearning_PoolHybrid', 0)) == 1
    ...
    for itech, parent in BASE_TECHS.items():
        ...
        if do_occ and np.isfinite(learned_occ):
            ...
            # path: 3_solve_oneyear.gms scales the wrapper's shared TES power-cycle
    ...
    # --- Write the per-year override gdx (re-read by 3_solve_oneyear.gms) ---
    outdir = os.path.join(case, 'outputs', 'nuclear_learning_data')
    os.makedirs(outdir, exist_ok=True)
    # Only write non-empty symbols; 3_solve_oneyear.gms loads learning_cost_cap only
...
def check(case):
    ...
    tes_island = int(sw.get('GSw_NuclearLearning_TESIsland', 0)) == 1
    wrapper_stortech = {}
    try:
