...
**Mixed-build stress test (2026-07-31, see notes doc §0.3 + addendum):** `z-ethan/mc/mixed_build_optimizer.ipynb` — per-draw optimizer over year-by-year SMR shares (2031–2050), granted drawn cross-tech spillover (U(0, 30%) each direction) and independent per-tech (LR, BOAK) draws conditioned on the honest orderings (SMR pricier at the anchor, SMR faster-learning). **At 6 schedules × 10,000 draws: mixing wins in 6 of 60,000 draws (0.01%), all on the smallest schedule, by ≤0.22% — a discounting-timing effect (0/6 survive undiscounted; 4/6 persist at zero spillover), not a traditional-first learning effect.** P(SMR is the cheaper pure program) rises 76%→89% with schedule ambition; S8 maps chart the decision boundary over the (LR gap, cost gap) plane. Winner-take-all validated for every at-scale schedule; regroup evidence alongside issue 8.

**100%-SMR case export (2026-07-31, see notes doc §0.4):** on the strength of the winner-take-all and mixed-build evidence, the ReEDS phase assumes deployment is 100% SMR. `z-ethan/mc/smr100_case_export.ipynb` regenerates the 18 cases under that assumption from the **main MC notebook's own sample space** (identical comonotone draw + seed streams, asserted against `mc_perdraw.npz`; 10,000/schedule): SMR = full program, large = intl-spillover-only counterfactual, P5/P50/P95 joint draws of SMR program cost per schedule. Exports `cases_nuclearlearning_smr100.csv` + all per-case cost/financing files, fully round-trip QA'd. This supersedes the winner-take-all `cases_nuclearlearning_mc.csv` set for the next phase (formally retire at the regroup).
