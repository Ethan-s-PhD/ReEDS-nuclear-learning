...
**Mixed-build stress test (2026-07-31, see notes doc §0.3):** `z-ethan/mc/mixed_build_optimizer.ipynb` — per-draw optimizer over year-by-year SMR shares, granted drawn cross-tech spillover (U(0, 30%) each direction) and decoupled BOAK draws (ρ_boak = 0.5, swept 0–1). **0/12,000 draws produce a mixed program beating the best pure strategy by >0.1%**; the closest call (−0.02%) persists at zero spillover (a corner near-tie, not a traditional-first effect). Winner-take-all validated under much weaker assumptions than the fragmentation figure's; regroup evidence alongside issue 8.
