...
**100%-SMR case export (2026-07-31, see notes doc §0.4):** on the strength of the winner-take-all and mixed-build evidence, the ReEDS phase assumes deployment is 100% SMR. `z-ethan/mc/smr100_case_export.ipynb` regenerates the 18 cases under that assumption (ordered independent draws, 10,000/schedule; SMR = full program, large = intl-spillover-only counterfactual; P5/P50/P95 joint draws of SMR program cost per schedule) and exports `cases_nuclearlearning_smr100.csv` + all per-case cost/financing files, fully round-trip QA'd. This supersedes the winner-take-all `cases_nuclearlearning_mc.csv` set for the next phase (formally retire at the regroup).
